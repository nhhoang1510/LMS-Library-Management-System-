<?php
	...
	if($query_string != '') {
		...
		$query = "SELECT book_id, book_name, book_author, book_no, book_category FROM books 
				  WHERE (book_name LIKE '%$query_string_escaped%' OR book_author LIKE '%$query_string_escaped%') 
				  ORDER BY book_name ASC";
	} else {
		$query = "SELECT book_id, book_name, book_author, book_no, book_category FROM books ORDER BY book_name ASC";
	}
	
	// Tính tổng số đầu sách và tổng số lượng
	$total_books_query = "SELECT COUNT(*) as total_titles, SUM(book_no) as total_quantity FROM books";
...
<body>
	...
	<div class="hello">
		...
		<h5 class="text-center"> <b><i>Quản lý thông tin sách trong hệ thống</i></b></h5>
	</div>
	...
	<div class="search-form">
		<form action="" method="GET">
			<div class="input-group">
				<input type="text" name="search" class="form-control" placeholder="Tìm kiếm sách..." value="<?php echo isset($_GET['search']) ? htmlspecialchars($_GET['search']) : ''; ?>">
		...
		<div class="table-responsive">
			<table class="table table-hover">
				...
				<tbody>
					...
					$query_display = "SELECT b.book_id, b.book_name, b.book_author, c.cat_name as book_category, b.book_no FROM books b JOIN category c ON b.book_category = c.cat_id";
					
					if(isset($_GET['search']) && !empty($_GET['search'])) {
						$search_term = mysqli_real_escape_string($connection, $_GET['search']);
						$query_display .= " WHERE b.book_name LIKE '%$search_term%' OR b.book_author LIKE '%$search_term%' OR c.cat_name LIKE '%$search_term%'";
					}

					$query_display .= " ORDER BY b.book_name ASC";
					...
					while ($row = mysqli_fetch_assoc($query_run)) {
						...
						<tr>
							...
							<td>
								<a href="edit_book.php?bn=<?php echo urlencode($row['book_name']); ?>" class="btn btn-edit btn-action">Sửa</a>
								<a href="delete_book.php?bn=<?php echo urlencode($row['book_name']); ?>" class="btn btn-delete btn-action" onclick="return confirm('Bạn có chắc chắn muốn xóa sách này?')">Xóa</a>
							</td>
						</tr>
						<?php
					}
